...
function getPushers(options) {
	...
	request({
      ...
      console.log('所有推流者', ret.data.pushersMap);
			var returnPushers = [],isInRoom = 0;
      var arr = [];
      arr = ret.data.pushersMap;
      console.log('所有推流者', arr);
      ...
      for (var j in arr) {
        //剔除自己
        if (arr[j].userID != accountInfo.userID) {
          returnPushers.push(arr[j]);
        } else {
					isInRoom = 1;
				}
      }
...
function proto_createRoom(options) {
  var groupId = getApp().globalData.groupId;
  console.log(groupId);
  // console.log(options);
  // console.log(options.data.groupId);
	request({
		url: 'create_room',
		data: {
			userID: accountInfo.userID,
